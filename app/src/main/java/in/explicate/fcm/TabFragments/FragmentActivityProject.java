...
import in.explicate.fcm.adapter.ActivityMyAdapter;
...
public class FragmentActivityProject extends Fragment {


    private SwipeRefreshLayout swipeRefreshLayout;
    private List<ActivityModel> list;
    private ActivityMyAdapter adapter;
    ...
    private void callApi() {
        ...
        Call<ActivityProjectAPIModel> call3 = apiInterface.getMyProjectActivities(AppClass.getUserId());
        call3.enqueue(new Callback<ActivityProjectAPIModel>() {
            @Override
            public void onResponse(Call<ActivityProjectAPIModel> call, Response<ActivityProjectAPIModel> response) {
                ...
                if(response.isSuccessful()){
                    ...
                    if(response.body()!=null){
                        ...
                        ActivityProjectAPIModel model=response.body();
            ...
            @Override
            public void onFailure(Call<ActivityProjectAPIModel> call, Throwable t) {
                call.cancel();
                hideLoader();
                swipeRefreshLayout.setRefreshing(false);
    ...
    private void showData(){
        ...
        adapter = new ActivityMyAdapter(list,getActivity());
    ...
    private void callApiSearchApi(String text) {
        ...
        APIInterface apiInterface = APIClient.getClient().create(APIInterface.class);
        Call<ActivityProjectAPIModel> call3 = apiInterface.searchMyProjectActivity(text,AppClass.getUserId());
        call3.enqueue(new Callback<ActivityProjectAPIModel>() {
            @Override
            public void onResponse(Call<ActivityProjectAPIModel> call, Response<ActivityProjectAPIModel> response) {
    ...
    private void getDetailApiCall(String text) {
        ...
        showLoader();
        APIInterface apiInterface = APIClient.getClient().create(APIInterface.class);
        Call<ActivityProjectDetailAPIModel> call3 = apiInterface.getProjectActivityDetails(text);
        call3.enqueue(new Callback<ActivityProjectDetailAPIModel>() {
            @Override
            public void onResponse(Call<ActivityProjectDetailAPIModel> call, Response<ActivityProjectDetailAPIModel> response) {
                ...
                switch (response.code()){
                    ...
                    case 200:
                        ...
                        if(response.body()!=null){
                            ...
                            ActivityProjectDetailAPIModel model=response.body();
                                ...
                                if(model.data!=null) {
                                    ...
                                    if (model.data.result.size() > 0) {
                                        ...
                                        for (int i = 0; i < model.data.result.size(); i++) {
                                            ...
                                            item.setSubject(model.data.result.get(i).subject);
                                            item.setDate(model.data.result.get(i).date);
                                            item.setProject(model.data.result.get(i).project);
            ...
            @Override
            public void onFailure(Call<ActivityProjectDetailAPIModel> call, Throwable t) {
                call.cancel();
                hideLoader();
                swipeRefreshLayout.setRefreshing(false);
                MyUtility.showAlertMessage(getActivity(),MyUtility.INTERNET_ERROR);
    ...
    private void showDetailedDialog(ActivityModel item){
        ...
        tvDepa.setText(item.getProject());
